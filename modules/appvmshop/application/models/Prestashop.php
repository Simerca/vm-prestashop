...
class Prestashop extends CI_Model {
	...
	private $cookie_key = "gSH8YT7tI7tzCFsYDLc80SvnPWZaqw5OROiVpYst82mDH1gGbq67bfvk";
	...
	function products($categorie = null, $active = TRUE, $shop = 1){
		$lang = $this->get_language_code($this->language());

		$this->db->select(array('product.id_product','product.id_category_default','product.reference','product.price','product_lang.name','product.id_tax_rules_group'));
		$this->db->from('product as product');
		$this->db->join('product_lang as product_lang', 'product_lang.id_product = product.id_product');
		$this->db->join('category_product as category', 'category.id_product = product.id_product');
        ...
        if($categorie != null){
            $this->db->where('category.id_category',$categorie);
        }
		// $this->db->where('id_shop', $shop);
	...
	function productsOrder($date = null, $id_shop = 1){
		...
		$sql = 'SELECT `order`.`reference`,`productLang`.`description_short`,`pd`.`product_id`,`order`.`id_shop`, `pd`.`product_name`, `pd`.`product_quantity`, `pd`.`product_weight`, `catLang`.`name` as `cat_name`,  `cat`.`id_category`, `order`.`date_add`, `cara`.`id_feature`, `caraLang`.`value`, `pdshp`.`id_category_default` FROM `prstshp_order_detail` as `pd`
		LEFT JOIN  `prstshp_product_shop` as `pdshp` ON `pdshp`.`id_product` = `pd`.`product_id`
		LEFT JOIN  `prstshp_category_product` as `cat` ON `pd`.`product_id` = `cat`.`id_product`
		LEFT JOIN `prstshp_category_lang` as `catLang` ON `pdshp`.`id_category_default` = `catLang`.`id_category`
		LEFT JOIN `prstshp_orders` as `order` ON `order`.`id_order` = `pd`.`id_order`
		JOIN `prstshp_product_lang` as `productLang` ON `productLang`.`id_product` = `pd`.`product_id`
		LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id` AND `cara`.`id_feature` = 15
		LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value`
		WHERE DATE(`order`.`date_retrait`) LIKE DATE("'.$date.'")
		GROUP BY `pd`.`id_order_detail`
		ORDER BY `pdshp`.`id_category_default` DESC, `pd`.`product_id` DESC';
		...
		LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id` AND `cara`.`id_feature` = 15
		LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value`
		WHERE DATE(`order`.`date_retrait`) LIKE DATE("'.$date.'") AND `order`.`boutique_retrait` = '.$id_shop.'
		GROUP BY `pd`.`id_order_detail`
		ORDER BY `pdshp`.`id_category_default` DESC, `pd`.`product_id` DESC';
	...
	function productsOrderGroupByRef($date = null, $id_shop = 2){

		$date = date('Y-m-d', strtotime($date));
		$lang = $this->get_language_code($this->language());

		$sql = 'SELECT `order`.`reference`,`pd`.`id_order_detail`,`pd`.`product_id`, `order`.`id_order`, `order`.`current_state`,`state`.`color`,`order`.`id_shop`,`status`.`name` as `status_name`, `order`.`date_retrait`, `order`.`heure_retrait`, `pd`.`product_name`, `pd`.`product_quantity`, `pd`.`product_weight`, `catLang`.`name` as `cat_name`, `cat`.`id_category`, `order`.`date_add`, `cara`.`id_feature`, `caraLang`.`value`, `customer`.`firstname` as `nom_client`, `customer`.`lastname` as `prenom_client` FROM `prstshp_order_detail` as `pd` JOIN `prstshp_category_product` as `cat` ON `pd`.`product_id` = `cat`.`id_product` JOIN `prstshp_category_lang` as `catLang` ON `cat`.`id_category` = `catLang`.`id_category` JOIN `prstshp_orders` as `order` ON `order`.`id_order` = `pd`.`id_order` JOIN `prstshp_customer` as `customer` ON `customer`.`id_customer` = `order`.`id_customer` LEFT OUTER JOIN `prstshp_order_state_lang` as `status` ON `status`.`id_order_state` = `order`.`current_state` LEFT OUTER JOIN `prstshp_order_state` as `state` ON `state`.`id_order_state` = `order`.`current_state` LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id` LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value` WHERE `order`.`boutique_retrait` = '.$id_shop.'  GROUP BY `pd`.`id_order_detail` ORDER BY `pd`.`product_id`="184" DESC,`pd`.`product_id` DESC, `order`.`date_retrait` DESC, `order`.`heure_retrait` ASC;';
	...
	function productsOrderGroupByRefLabo($date = null, $id_shop = 2){

		$date = date('Y-m-d', strtotime($date));
		$lang = $this->get_language_code($this->language());

		$sql = 'SELECT `order`.`reference`,`pd`.`id_order_detail`,`pd`.`product_id`, `order`.`id_order`, `order`.`current_state`,`state`.`color`,`order`.`id_shop`,`status`.`name` as `status_name`, `order`.`date_retrait`, `order`.`heure_retrait`, `pd`.`product_name`, `pd`.`product_quantity`, `pd`.`product_weight`, `catLang`.`name` as `cat_name`, `cat`.`id_category`, `order`.`date_add`, `cara`.`id_feature`, `caraLang`.`value`, `customer`.`firstname` as `nom_client`, `customer`.`lastname` as `prenom_client` FROM `prstshp_order_detail` as `pd` JOIN `prstshp_category_product` as `cat` ON `pd`.`product_id` = `cat`.`id_product` JOIN `prstshp_category_lang` as `catLang` ON `cat`.`id_category` = `catLang`.`id_category` JOIN `prstshp_orders` as `order` ON `order`.`id_order` = `pd`.`id_order` JOIN `prstshp_customer` as `customer` ON `customer`.`id_customer` = `order`.`id_customer` LEFT OUTER JOIN `prstshp_order_state_lang` as `status` ON `status`.`id_order_state` = `order`.`current_state` LEFT OUTER JOIN `prstshp_order_state` as `state` ON `state`.`id_order_state` = `order`.`current_state` LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id` LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value` WHERE `order`.`boutique_retrait` = '.$id_shop.' AND `cara`.`id_feature` = 16 GROUP BY `pd`.`id_order_detail` ORDER BY `pd`.`product_id`="184" DESC,`pd`.`product_id` DESC, `order`.`date_retrait` DESC, `order`.`heure_retrait` ASC;';
	...
	function productsOrderRef($id){

		$lang = $this->get_language_code($this->language());

		$sql = 'SELECT `order`.`reference`,`pd`.`product_id`,`pd`.`is_check`,`pd`.`is_refund`,`order`.`id_shop`,`pd`.`id_order_detail`, `pd`.`product_name`, `pd`.`product_quantity`, `pd`.`product_weight`, `catLang`.`name` as `cat_name`,  `cat`.`id_category`, `order`.`date_add`, `cara`.`id_feature`, `caraLang`.`value` FROM `prstshp_order_detail` as `pd`
		JOIN  `prstshp_category_product` as `cat` ON `pd`.`product_id` = `cat`.`id_product`
		JOIN `prstshp_category_lang` as `catLang` ON `cat`.`id_category` = `catLang`.`id_category`
		JOIN `prstshp_orders` as `order` ON `order`.`id_order` = `pd`.`id_order`
		LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id`
		LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value`
		WHERE `order`.`reference` = "'.$id.'"
		GROUP BY `pd`.`id_order_detail`
	...
	function productsOrderRefLabo($id){
		...
		LEFT OUTER JOIN `prstshp_feature_product` as `cara` ON `cara`.`id_product` = `pd`.`product_id`
		LEFT OUTER JOIN `prstshp_feature_value_lang` as `caraLang` ON `caraLang`.`id_feature_value` = `cara`.`id_feature_value`
		WHERE `order`.`reference` = "'.$id.'" AND `cara`.`id_feature` = 16
		GROUP BY `pd`.`id_order_detail`
		ORDER BY `cat`.`id_category`';
	...
	function updateOrderDetail($id){
		...
		$sql = 'SELECT `detail`.`id_order` FROM `prstshp_order_detail` as `detail`
		JOIN `prstshp_order_detail` as `order` ON `order`.`id_order` = `detail`.`id_order`
		WHERE `detail`.`id_order_detail` = 10
		GROUP BY `detail`.`id_order`';

		$order_detail = $this->db->where('id_order_detail', $id)
									->get('order_detail')
									->row();
		
		if($order_detail->is_check == 1){
			$order_detailUpdate = array(
				'is_check'=>0
			);
		}else{
				$order_detailUpdate = array(
		...
		$this->db->where('id_order_detail',$id)
					->update('order_detail', $order_detailUpdate);
	...
	public function create_cart(){
		...
		$cart = $this->db->where('id_customer', $this->session->userdata('id_customer'))
							->get('cart')
							->row();
		if($cart){
	...
	public function login($user, $password){
		...
		$passwd = md5($this->cookie_key .$password);

		$login = $this->db->where('email', $user)
					->where('passwd', $passwd)
					->get('customer')
					->row();
		if($login){
