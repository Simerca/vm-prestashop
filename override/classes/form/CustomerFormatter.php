...
class CustomerFormatterCore implements FormFormatterInterface
{
    ...
    public function getFormat()
    {
        ...
        $format['18y'] = (new FormField())
            ->setName('18y')
            ->setType('checkbox')
            ->setLabel(
                $this->translator->trans(
                    'Je certifie avoir plus de 18 ans', [], 'Shop.Theme.Customeraccount'
                )
            )->setRequired(true)
        ;
        $format['rgpd'] = (new FormField())
            ->setName('rgpd')
            ->setType('checkbox')
            ->setLabel(
                $this->translator->trans(
                    'En créant mon compte client, j\'accepte que mes données personnelles soient utilisées par Vents&Marées pour le traitement de mes commandes. Depuis la loi “informatique et libertés” du 6 janvier 1978 modifiée, vous bénéficiez d’un droit d’accès et de rectification aux informations qui vous concernent. Si vous souhaitez exercer ce droit et obtenir communication des informations vous concernant, veuillez nous envoyer une demande via notre formulaire de contact', [], 'Shop.Theme.Customeraccount'
                )
            )->setRequired(true)
        ;
