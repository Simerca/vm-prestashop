...
class AdminOrdersControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->_select = '
        a.id_currency,
        a.boutique_facturation,
        shopFac.name as shop_facturation,
        shopRetrait.name as shop_retrait,
        a.date_retrait,
        a.heure_retrait,
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'shop` as shopFac ON (shopFac.`id_shop` = a.`boutique_facturation`)
		LEFT JOIN `' . _DB_PREFIX_ . 'shop` as shopRetrait ON (shopRetrait.`id_shop` = a.`boutique_retrait`)
        ...
        $this->fields_list = array_merge($this->fields_list, array(
            ...
            'shop_facturation' => array(
                'title' => $this->trans('Boutique Facturation', array(), 'Admin.Global'),
            ),
            'shop_retrait' => array(
                'title' => $this->trans('Boutique Retrait', array(), 'Admin.Global'),
            ),
            'date_retrait' => array(
                'title' => $this->trans('Date Retrait', array(), 'Admin.Global'),
            ),
            'heure_retrait' => array(
                'title' => $this->trans('Heure retrait', array(), 'Admin.Global'),
            ),
            'osname' => array(
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitShippingNumber') && isset($order)) {
            ...
            ($id_order_state = Tools::getValue('id_order_state')) && Validate::isModuleName($module_name)) {
                ...
                $_SESSION['boutique_facturation'] = Tools::getValue('boutique_facturation');
            ...
            if ($this->access('edit')) {
                ...
                if (($bad_delivery = (bool) !Address::isCountryActiveById((int) $cart->id_address_delivery))
                    ...
                    $date = Tools::getValue('date_retrait');
                    ...
                    $payment_module->validateOrder(
                        ...
                        substr($employee->firstname, 0, 1) . '. ' . $employee->lastname, array(), null, false, $cart->secure_key,null,Tools::getValue('heure_retrait'),Tools::getValue('boutique_retrait'),date('Y-m-d',strtotime($date))
                    );
                    if ($payment_module->currentOrder) {
                        ...
                        echo "<pre>";
                        var_dump($payment_module->currentOrder);
                        echo "</pre>";
                        $date = Tools::getValue('date_retrait');
                        $date = str_replace('/','-',$date);
                        Db::getInstance()->update('orders', array(
                            'date_retrait'      => pSQL(date('Y-m-d',strtotime($date))),
                            'heure_retrait'      => pSQL(Tools::getValue('heure_retrait')),
                            'boutique_facturation'      => pSQL(Tools::getValue('boutique_facturation')),
                            'boutique_retrait'      => pSQL(Tools::getValue('boutique_retrait')),
                            'ticket_restaurant'      => pSQL(Tools::getValue('ticket_restaurant')),
                            'id_shop'      => pSQL(Tools::getValue('boutique_facturation')),
    ...
    public function ajaxProcessSearchProducts()
    {
        ...
        if ($products = Product::searchByName((int) $this->context->language->id, pSQL(Tools::getValue('product_search')))) {
            foreach ($products as &$product) {
                ...
                // echo "</pre>";
                //Description Courte
                $product['description_courte'] = strip_tags($productObj->description_short);

                $link = new Link;//because getImageLInk is not static function

                $img = $productObj->getCover($productObj->id);
                $image_type = 'small_default';
                $product['image'] = 'https://'.$link->getImageLink(isset($productObj->link_rewrite) ? $productObj->link_rewrite : $productObj->name, (int)$img['id_image'], $image_type);
