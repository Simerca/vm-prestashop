...
class OrderControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if(isset($_GET['date_retrait'])){

            $cart->boutique_retrait = $id_shop;
            $cart->date_retrait = $_GET['date_retrait'];
            $_GET['date_retrait'] = str_replace('/','-', $_GET['date_retrait']);
            $cart->heure_retrait = $_GET['heure_retrait'];

            Db::getInstance()->update('cart',array(
                'boutique_retrait'=>$_GET['boutique_retrait'],
                'date_retrait'=>date('Y-m-d', strtotime($_GET['date_retrait'])),
                'heure_retrait'=>$_GET['heure_retrait']
            ),'id_cart ='.$this->context->cart->id);


            if(Db::getInstance()->insert('cart_date',array(
                'id_cart'=>$this->context->cart->id,
                'date_retrait'=>$_GET['date_retrait'],
                'heure_retrait'=>$_GET['heure_retrait']
            ))){
                $this->context->smarty->assign([
                    'hide_calendar' => true,
        ...
        $cartUnformated = Db::getInstance()->getRow($sql);

        $this->context->smarty->assign([
            'cartUnformated' => $cartUnformated,
        ]);
